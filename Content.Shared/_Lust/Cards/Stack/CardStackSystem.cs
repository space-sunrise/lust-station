...
using Content.Shared._Lust.Cards.Deck;
...
/// <summary>
/// A stack of cards: insert, remove, shuffle, flip, join and draw.
...
public sealed class CardStackSystem : EntitySystem
{
    ...
    [Dependency] private readonly CardHandSystem _cardHandSystem = default!;    [Dependency] private readonly SharedHandsSystem _hands = default!;
    ...
    private void OnAlternativeVerb(EntityUid uid, CardStackComponent component, GetVerbsEvent<AlternativeVerb> args)
    {
        ...
        else if (TryComp(args.Using, out CardComponent? usingCard)) // single card interaction
        {
            args.Verbs.Add(new AlternativeVerb()
            {
                Text = Loc.GetString("card-verb-join"),
    ...
    // hacky misuse of the activation verb, but allows us a separate way to draw cards without needing additional buttons and event fiddling
    ...
    public void TransferNLastCardFromStacks(EntityUid user, int n, EntityUid first, CardStackComponent firstComp, EntityUid second, CardStackComponent secondComp)
    {
        ...
        var cards = firstComp.Cards.TakeLast(n).ToList(); // make a copy we don't munge during iteration
    ...
    private void OnActivate(EntityUid uid, CardStackComponent component, ActivateInWorldEvent args)
    {
        ...
        {
            // Don't draw a card from a stack onto itself.
            args.Handled = true;
            return;
        }
