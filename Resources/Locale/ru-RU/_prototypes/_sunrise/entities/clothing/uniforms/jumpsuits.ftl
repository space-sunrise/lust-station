ent-ClothingUniformJumpsuitSuperstarCop = форма полицейского суперзвезды
    .desc = Расклёшенные брюки и грязная рубашка, которая могла бы быть стильной, пока кто-то не помочился в подмышки. Это одежда суперзвезды.
ent-ClothingUniformJumpsuitAerostatic = костюм воздухоплавателя
    .desc = Свежий и хорошо выглаженный костюм; профессиональный, удобный и удивительно авторитетный.
ent-ClothingUniformJumpsuitCentcomOfficerBlack = униформа офицера специальных операций
    .desc = Чёрная униформа Офицера Специальных Операций, ничего такого. Хотя... Если у вас есть время читать это, то уже поздно...
ent-ClothingUniformJumpsuitCentcomAdmiral = униформа адмирала
    .desc = Униформа, которую носят офицеры в звании адмирала флота Nanotrasen.
ent-MagistratUniformSuit = костюм магистрата
    .desc = Увольте пару срочников!
ent-ClothingUniformJumpsuitNtrep = костюм представителя Qillu
    .desc = Этот костюм лучше не видеть.
ent-ClothingUniformJumpsuitNtrepFormal = праздничный костюм представителя Qillu
    .desc = Этот костюм лучше не видеть.
ent-ClothingUniformJumpsuitBlueShield = комбинезон офицера «Синий щит»
    .desc = На стиле.
ent-ClothingUniformJumpsuitNTRG = униформа ИСН
    .desc = Черная, как космическая бездна, и строгая, как сам Закон, униформа ИСН — это не просто одежда, а символ абсолютного авторитета.
ent-ClothingUniformJumpsuitPrisonerGrey = комбинезон заключённого
    .desc = Почему он не оранжевый?
ent-ClothingUniformJumpsuitPrisonGuard = комбинезон охранника
    .desc = Почему он не красный?
ent-ClothingUniformJumpsuitPilot = комбинезон пилота Службы Безопасности
    .desc = Лёгкий комбинезон, рекомендуемый 9 пилотами НТ из 10.
ent-ClothingUniformJumpsuitIAA = рубашка с галстуком
    .desc = Костюм из льняной белой рубашки с синим галстуком и брюк. Выглядит роскошно.
ent-ClothingUniformJumpsuitHoSOld = старый комбинезон главы службы безопасности
    .desc = Довольно старая вещь для этих времён, навевающая старые воспоминания...
ent-ClothingUniformJumpsuitUSSPBlack = старый комбинезон СССП
    .desc = Старая вещица, прямиком из СССП.
ent-ClothingUniformJumpsuitSkeleton = комбинезон скелета
    .desc = Чёрный комбинезон с напечатанным на нем белым узором из костей. Жуть!
ent-ClothingUniformJumpsuitBarber = костюм парикмахера
    .desc = Как же он хорош.
ent-ClothingUniformJumpsuitAdjutant = комбинезон адъютанта
    .desc = Сшито на совесть из дышащей ткани, что позволит вам быть на ногах всю смену.
ent-ClothingUniformJumpsuitAdidasBlack = чёрная спортивка СССП
    .desc = Отлично сшитый комплект штанов, футболки и ветровки. Имеет отличительные особенности в две полосы, именной принт "Красный Серп".
ent-ClothingUniformJumpsuitAdidasWhite = белая спортивка СССП
    .desc = Белый вариант популярной спортивной формы олимпийцев СССП, имеет две чёрные полосы на каждой части комплекта. Модно, стильно и ярко!
ent-ClothingUniformJumpskirtChristmas = Рождественский женский комбинезон
    .desc = Украшен в тонах рождества, ушит специально для красивой части станции.
ent-ClothingUniformJumpsuitBlackturtleneck = чёрная водолазка
    .desc = Стильная водолазка с черными брюками. На бирке написано "Sokol & Kendrick production".
ent-ClothingUniformMailCarrier = униформа почтальона
    .desc = Враг для собак номер один.
ent-ClothingUniformJumpsuitMantis = униформа криминального богомола
    .desc = Эта униформа, созданная по образцу старинной пехотной униформы, обеспечивает превосходную мобильность в напряженных ситуациях.
ent-ClothingUniformJumpsuitGalaxyRed = красный галактический костюм
    .desc = Истинная красная форма для тех, кто ведет бизнес в галактических масштабах.
ent-ClothingUniformJumpsuitGalaxyBlue = синий галактический костюм
    .desc = Истинная синия форма для тех, кто ведет бизнес в галактических масштабах.
ent-ClothingUniformJumpsuitSecSummer = летняя униформа охраны
    .desc = Свободный и повседневный вариант стандартной униформы охранника в жаркие деньки.
ent-ClothingUniformJumpsuitExplorer = форма исследователя
    .desc = Довольно прочный комбинезон для походов к звездам.
ent-ClothingUniformTShirtKhakiPants = однотонная футболка и брюки цвета хаки
    .desc = Обычная, но функциональная одежда.
ent-ClothingUniformJumpsuitAtmosSyndie = Акробатический комбинезон
    .desc = { ent-ClothingUniformJumpsuitAtmos.desc }
ent-ClothingUniformJumpsuitArmouredBlack = чёрный адвокатский костюм
    .desc = Казалось бы, простой деловой костюм... пока не отлетела пуля...
ent-ClothingUniformJumpsuitBlueshieldJS = комбинезон синего щита
    .desc = { ent-ClothingUniformBase.desc }
ent-ClothingUniformJumpsuitMiningSpecialist = комбинезон специалиста по добыче
    .desc = Костюм, передававшийся из поколения в поколение. За ним тянется тонкий слой пыли.
ent-ClothingUniformJumpsuitAbductor = униформа абдуктора
    .desc = { ent-UnsensoredClothingUniformBase.desc }
ent-UniformSecurityShortsRed = шорты сб
    .desc = Это шорты, а не боксёры.
ent-ClothingUniformJumpsuitCapTurtleneck = черенок капитана
    .desc = Истинный черенок капитана.
ent-ClothingUniformJumpsuitHoPTurtleneck = черенок начальника персонала
    .desc = Это черенок начальника персонала.
ent-ClothingUniformJumpsuitHoSBlack = чёрный костюм начальника охраны
    .desc = Это чёрный костюм начальника охраны.
ent-ClothingUniformJumpsuitRDClassical = классический костюм директора исследований
    .desc = Это классический костюм директора исследований.
